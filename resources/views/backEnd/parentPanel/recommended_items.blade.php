                                ...
                                <tbody>
                                    @foreach($items as $assigned)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('parent-recommended-items-cart') }}" class="btn btn-sm primary-btn fix-gr-bg mb-1">
                                                    Go to cart
                                                </a>
                                            </td>
                                        </tr>
...
<script>
    function addToCart(itemId) {
        $.ajax({
            type: 'POST',
            url: '{{ route("add-recommended-item", ":id") }}'.replace(':id', itemId),
            data: {
                _token: '{{ csrf_token() }}',
                item_id: itemId
            },
            success: function(response) {
                if(response.success) {
                    ...
                    setTimeout(function() {
                        window.location.href = '{{ route("parent-recommended-items-cart") }}';
                    }, 1500);
                } else {
